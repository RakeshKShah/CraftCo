...
CASE_SUFFIX="$(date +%s)-$$"
USER_ID="user-inactive-9-${CASE_SUFFIX}"
SELLER_ID="seller-inactive-9-${CASE_SUFFIX}"
PRODUCT_ID="prod-inactive-9-${CASE_SUFFIX}"
USER_EMAIL="inactive-${CASE_SUFFIX}@example.com"
RESPONSE_FILE="/tmp/inactive_seller_rejected_by_middleware_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/inactive_seller_rejected_by_middleware_${CASE_SUFFIX}.status"
...
INSERT INTO users (id, email, password_hash, role, status) VALUES ('${USER_ID}', '${USER_EMAIL}', 'hash', 'SELLER', 'PENDING');
INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'Inactive Store ${CASE_SUFFIX}', 'bio');
INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible)
VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Inactive Product ${CASE_SUFFIX}', 'desc', 'crafts', 1200, 2, '[]'::jsonb, 'ACTIVE', true);
SQL
TOKEN="$(create_jwt "{\"id\":\"${USER_ID}\",\"email\":\"${USER_EMAIL}\",\"role\":\"SELLER\",\"status\":\"PENDING\"}")"
...
[ "$DB_ROW" = "ACTIVE|true" ]
echo "CODEVALID_TEST_ASSERTION_OK:inactive_seller_rejected_by_middleware"
