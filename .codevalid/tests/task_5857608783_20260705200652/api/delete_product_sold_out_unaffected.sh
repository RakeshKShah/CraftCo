...
CASE_SUFFIX="$(date +%s)-$$"
USER_ID="user-seller-SO-${CASE_SUFFIX}"
SELLER_ID="seller-SO-${CASE_SUFFIX}"
PRODUCT_ID="prod-soldout-7-${CASE_SUFFIX}"
USER_EMAIL="seller-so-${CASE_SUFFIX}@example.com"
RESPONSE_FILE="/tmp/delete_product_sold_out_unaffected_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/delete_product_sold_out_unaffected_${CASE_SUFFIX}.status"
...
INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'Sold Out Store ${CASE_SUFFIX}', 'bio');
INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible)
VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Sold Out Product ${CASE_SUFFIX}', 'desc', 'crafts', 800, 0, '[]'::jsonb, 'SOLD_OUT', true);
SQL
...
echo "CODEVALID_TEST_ASSERTION_OK:delete_product_sold_out_unaffected"
