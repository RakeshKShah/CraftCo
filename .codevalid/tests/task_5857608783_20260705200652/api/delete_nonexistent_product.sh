...
CASE_SUFFIX="$(date +%s)-$$"
USER_ID="user-seller-X-${CASE_SUFFIX}"
SELLER_ID="seller-X-${CASE_SUFFIX}"
PRODUCT_ID="prod-nonexistent-9-${CASE_SUFFIX}"
USER_EMAIL="seller-x-${CASE_SUFFIX}@example.com"
RESPONSE_FILE="/tmp/delete_nonexistent_product_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/delete_nonexistent_product_${CASE_SUFFIX}.status"
...
cleanup_db() {
  psql "$DATABASE_URL" -c "DELETE FROM seller_profiles WHERE id = '${SELLER_ID}'; DELETE FROM users WHERE id = '${USER_ID}';" >/dev/null
}
...
INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'Store X ${CASE_SUFFIX}', 'bio');
