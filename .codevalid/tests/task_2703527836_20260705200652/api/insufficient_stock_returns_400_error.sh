...
CASE_SUFFIX="$(date +%s)-$$"
BUYER_ID="buyer-stock-${CASE_SUFFIX}"
SELLER_USER_ID="seller-user-stock-${CASE_SUFFIX}"
SELLER_PROFILE_ID="seller-profile-stock-${CASE_SUFFIX}"
PRODUCT_ID="prod-300-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/insufficient_stock_returns_400_error_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/insufficient_stock_returns_400_error_${CASE_SUFFIX}.status"
...
INSERT INTO "SellerProfile" (id, "userId", "storeName", bio, status, "createdAt") VALUES
  ('${SELLER_PROFILE_ID}', '${SELLER_USER_ID}', 'Stock Store ${CASE_SUFFIX}', 'bio', 'ACTIVE', NOW());
INSERT INTO "Product" (id, title, description, price_cents, stock_qty, visible, status, "sellerId", category, "createdAt") VALUES
  ('${PRODUCT_ID}', 'Limited Product ${CASE_SUFFIX}', 'desc', 2000, 3, true, 'ACTIVE', '${SELLER_PROFILE_ID}', 'general', NOW());
SQL
...
[ "$ORDER_COUNT" = '0' ]
echo 'CODEVALID_TEST_ASSERTION_OK:insufficient_stock_returns_400_error'
