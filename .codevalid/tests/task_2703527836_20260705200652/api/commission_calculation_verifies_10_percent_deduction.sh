...
CASE_SUFFIX="$(date +%s)-$$"
BUYER_ID="buyer-commission-${CASE_SUFFIX}"
SELLER_USER_ID="seller-user-commission-${CASE_SUFFIX}"
SELLER_PROFILE_ID="seller-profile-commission-${CASE_SUFFIX}"
PRODUCT_ID="prod-200-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/commission_calculation_verifies_10_percent_deduction_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/commission_calculation_verifies_10_percent_deduction_${CASE_SUFFIX}.status"
...
INSERT INTO "SellerProfile" (id, "userId", "storeName", bio, status, "createdAt") VALUES
  ('${SELLER_PROFILE_ID}', '${SELLER_USER_ID}', 'Commission Store ${CASE_SUFFIX}', 'bio', 'ACTIVE', NOW());
INSERT INTO "Product" (id, title, description, price_cents, stock_qty, visible, status, "sellerId", category, "createdAt") VALUES
  ('${PRODUCT_ID}', 'Commission Product ${CASE_SUFFIX}', 'desc', 10000, 20, true, 'ACTIVE', '${SELLER_PROFILE_ID}', 'general', NOW());
SQL
...
[ "$ORDER_CHECK" = '30000|3000|PAID' ]
ITEM_CHECK="$(psql "$DATABASE_URL" -At -F '|' -c "SELECT seller_payout_cents, qty FROM \"OrderItem\" WHERE \"orderId\" = '${ORDER_ID}' AND \"productId\" = '${PRODUCT_ID}';")"
[ "$ITEM_CHECK" = '27000|3' ]
echo 'CODEVALID_TEST_ASSERTION_OK:commission_calculation_verifies_10_percent_deduction'
