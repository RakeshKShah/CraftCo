...
CASE_SUFFIX="$(date +%s)-$$"
BUYER_ID="buyer-decrement-${CASE_SUFFIX}"
SELLER_USER_ID="seller-user-decrement-${CASE_SUFFIX}"
SELLER_PROFILE_ID="seller-profile-decrement-${CASE_SUFFIX}"
PRODUCT_ID="prod-stock-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/stock_decremented_after_successful_checkout_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/stock_decremented_after_successful_checkout_${CASE_SUFFIX}.status"
...
INSERT INTO "SellerProfile" (id, "userId", "storeName", bio, status, "createdAt") VALUES
  ('${SELLER_PROFILE_ID}', '${SELLER_USER_ID}', 'Stock Sync Store ${CASE_SUFFIX}', 'bio', 'ACTIVE', NOW());
INSERT INTO "Product" (id, title, description, price_cents, stock_qty, visible, status, "sellerId", category, "createdAt") VALUES
  ('${PRODUCT_ID}', 'Stock Product ${CASE_SUFFIX}', 'desc', 1000, 50, true, 'ACTIVE', '${SELLER_PROFILE_ID}', 'general', NOW());
SQL
INITIAL_STOCK="$(psql "$DATABASE_URL" -At -c "SELECT stock_qty FROM \"Product\" WHERE id = '${PRODUCT_ID}';")"
[ "$INITIAL_STOCK" = '50' ]
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  ...
  --data "{\"items\":[{\"product_id\":\"${PRODUCT_ID}\",\"qty\":7}]}" > "$STATUS_FILE"
...
[ "$UPDATED_STOCK" = '43' ]
echo 'CODEVALID_TEST_ASSERTION_OK:stock_decremented_after_successful_checkout'
