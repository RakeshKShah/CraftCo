...
psql "$DATABASE_URL" -c "INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'New Arrivals', 'Fresh products');" >/dev/null
psql "$DATABASE_URL" -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible) VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'New Widget', 'Brand new widget', 'HOME', 1500, 5, '[]'::jsonb, 'ACTIVE', true);" >/dev/null
...
[ "$STATUS" = "200" ]
grep -F '"store_name":"New Arrivals"' "$RESPONSE_FILE" >/dev/null
grep -F '"bio":"Fresh products"' "$RESPONSE_FILE" >/dev/null
grep -F '"status":"ACTIVE"' "$RESPONSE_FILE" >/dev/null
grep -F '"title":"New Widget"' "$RESPONSE_FILE" >/dev/null
grep -F '"orders":[]' "$RESPONSE_FILE" >/dev/null
grep -F '"total_earnings_cents":0' "$RESPONSE_FILE" >/dev/null

echo "CODEVALID_TEST_ASSERTION_OK:seller_with_no_orders"
