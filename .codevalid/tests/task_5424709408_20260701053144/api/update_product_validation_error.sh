...
TOKEN_FILE="/tmp/update_product_validation_error_${CASE_SUFFIX}.token"
RESPONSE_FILE="/tmp/update_product_validation_error_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/update_product_validation_error_${CASE_SUFFIX}.status"
cleanup() {
...
psql "$DATABASE_URL" -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible) VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Validation Title', 'Validation description', 'BOOKS', 3100, 6, '[]'::jsonb, 'ACTIVE', true);"
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  ...
  --data '{"price_cents":-100}' > "$STATUS_FILE"
