...
# Given — seed an admin and a pending seller
...
INSERT INTO users (id, email, password_hash, role, status, created_at)
VALUES ('${SELLER_USER_ID}', 'seller-${TEST_ID}-${CASE_SUFFIX}@example.com', '${HASH}', 'SELLER', 'PENDING', NOW());
INSERT INTO seller_profiles (id, user_id, store_name, bio)
VALUES ('${SELLER_ID}', '${SELLER_USER_ID}', 'Missing Status Store ${CASE_SUFFIX}', 'Validation bio');
SQL
...
# When — admin sends an empty body without the required status field
...
# Then — endpoint returns 400 validation error and seller remains pending
[ "$(cat "$STATUS_FILE")" = "400" ]
...
echo "CODEVALID_TEST_ASSERTION_OK:missing_status_field_rejected"
