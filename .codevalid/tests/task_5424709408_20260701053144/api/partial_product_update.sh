...
TOKEN_FILE="/tmp/partial_product_update_${CASE_SUFFIX}.token"
RESPONSE_FILE="/tmp/partial_product_update_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/partial_product_update_${CASE_SUFFIX}.status"
cleanup() {
...
psql "$DATABASE_URL" -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible) VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Original', 'Original Desc', 'HOME_GOODS', 1000, 10, '[\"https://example.com/partial.jpg\"]', 'ACTIVE', true);"
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  ...
  --data '{"price_cents":1500}' > "$STATUS_FILE"

# Then
STATUS="$(cat "$STATUS_FILE")"
[ "$STATUS" = "200" ]
grep -F '"id":"'"$PRODUCT_ID"'"' "$RESPONSE_FILE" >/dev/null
grep -F '"title":"Original"' "$RESPONSE_FILE" >/dev/null
...
DB_ROW="$(psql "$DATABASE_URL" -t -A -c "SELECT title || '|' || description || '|' || price_cents::text || '|' || stock_qty::text FROM products WHERE id='${PRODUCT_ID}';")"
[ "$DB_ROW" = 'Original|Original Desc|1500|10' ]
echo "CODEVALID_TEST_ASSERTION_OK:partial_product_update"
