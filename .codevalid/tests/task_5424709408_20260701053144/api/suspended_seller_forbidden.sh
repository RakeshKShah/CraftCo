...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="suspended_seller_forbidden"
...
curl -sS -X POST "$BASE_URL/auth/register" \
  ...
  -d "{\"email\":\"${USER_EMAIL}\",\"password\":\"${USER_PASSWORD}\",\"role\":\"SELLER\",\"storeName\":\"Suspended Store ${CASE_SUFFIX}\",\"bio\":\"suspended seller\"}" \
  > "$REGISTER_FILE"
...
psql "$DATABASE_URL" -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible) VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Suspended Delete ${CASE_SUFFIX}', 'suspended seller product', 'tools', 1750, 2, '[]'::jsonb, 'ACTIVE', true);" >/dev/null
