...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="admin_approve_pending_seller"
...
SELLER_ID="seller-abc123-${CASE_SUFFIX}"
SEEDED_PRODUCT_ID="prod-approval-seeded-${CASE_SUFFIX}"
...
cleanup() {
  ...
  psql "$DATABASE_URL" -c "DELETE FROM products WHERE id='${SEEDED_PRODUCT_ID}';" >/dev/null 2>&1 || true
...
# Given — seed an admin and a pending seller account with store profile and hidden existing product
...
INSERT INTO users (id, email, password_hash, role, status, created_at)
VALUES ('${SELLER_USER_ID}', '${SELLER_EMAIL}', '${HASH}', 'SELLER', 'PENDING', NOW());
INSERT INTO seller_profiles (id, user_id, store_name, bio)
VALUES ('${SELLER_ID}', '${SELLER_USER_ID}', 'Pending Store ${CASE_SUFFIX}', 'Pending bio ${CASE_SUFFIX}');
INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible, created_at)
VALUES ('${SEEDED_PRODUCT_ID}', '${SELLER_ID}', 'Pending Product ${CASE_SUFFIX}', 'Hidden while pending', 'tools', 1500, 3, '[]', 'ACTIVE', false, NOW());
SQL
...
# When — admin approves the seller, then the seller attempts to publish a product listing
...
# Then — response is 200, seller is ACTIVE, seeded products become visible, and publishing succeeds
[ "$(cat "$STATUS_FILE")" = "200" ]
...
[ "$(psql "$DATABASE_URL" -t -A -c "SELECT visible FROM products WHERE id='${SEEDED_PRODUCT_ID}';")" = "t" ]
...
echo "CODEVALID_TEST_ASSERTION_OK:admin_approve_pending_seller"
