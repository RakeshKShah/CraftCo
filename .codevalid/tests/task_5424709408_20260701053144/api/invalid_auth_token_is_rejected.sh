...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="invalid_auth_token_is_rejected"
...
curl -sS -X POST "$BASE_URL/auth/register" \
  ...
  -d "{\"email\":\"${USER_EMAIL}\",\"password\":\"${USER_PASSWORD}\",\"role\":\"SELLER\",\"storeName\":\"Invalid Token Store ${CASE_SUFFIX}\",\"bio\":\"invalid token test\"}" \
  > "$REGISTER_FILE"
...
psql "$DATABASE_URL" -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible) VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Invalid Token Product ${CASE_SUFFIX}', 'target for invalid token', 'tools', 1900, 2, '[]'::jsonb, 'ACTIVE', true);" >/dev/null
...
echo "CODEVALID_TEST_ASSERTION_OK:invalid_auth_token_is_rejected"
