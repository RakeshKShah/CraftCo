...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="seller_not_found_error"
ADMIN_ID="admin-${TEST_ID}-${CASE_SUFFIX}"
ADMIN_EMAIL="${ADMIN_ID}@example.com"
ADMIN_PASSWORD='AdminPass123!'
MISSING_SELLER_ID="nonexistent-seller-999-${CASE_SUFFIX}"
...
# Given — seed only an active administrator and ensure the seller id does not exist
HASH='$2b$10$DOWSDR9Kt5I9zQJmlp7iUuY8j5B5P6M8JrIoYNewc19hXtOD87o5e'
psql "$DATABASE_URL" -c "INSERT INTO users (id, email, password_hash, role, status, created_at) VALUES ('${ADMIN_ID}', '${ADMIN_EMAIL}', '${HASH}', 'ADMIN', 'ACTIVE', NOW());" >/dev/null
...
# When — admin attempts to update a seller that does not exist
...
# Then — endpoint returns 404 Seller not found and no database changes occur for that seller id
[ "$(cat "$STATUS_FILE")" = "404" ]
jq -e '.error == "Seller not found"' "$RESPONSE_JSON" >/dev/null
[ "$(psql "$DATABASE_URL" -t -A -c "SELECT COUNT(*) FROM seller_profiles WHERE id='${MISSING_SELLER_ID}';")" = "0" ]

echo "CODEVALID_TEST_ASSERTION_OK:seller_not_found_error"

# Cleanup — remove seeded admin
