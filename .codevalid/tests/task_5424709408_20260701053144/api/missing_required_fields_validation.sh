...
CASE_SUFFIX="$(date +%s)-$$"
USER_ID="user-validation-missing-${CASE_SUFFIX}"
SELLER_ID="seller-validation-missing-${CASE_SUFFIX}"
EMAIL="validation-missing-${CASE_SUFFIX}@example.com"
RESPONSE_FILE="/tmp/missing_required_fields_validation_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/missing_required_fields_validation_${CASE_SUFFIX}.status"
TOKEN_FILE="/tmp/missing_required_fields_validation_${CASE_SUFFIX}.token"
cleanup() {
...
psql "$DATABASE_URL" -c "INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'Validation Store ${CASE_SUFFIX}', 'Validation bio');"
