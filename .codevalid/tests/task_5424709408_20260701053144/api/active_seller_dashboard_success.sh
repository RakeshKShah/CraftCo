...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="active_seller_dashboard_success"
USER_ID="user_${TEST_ID}_${CASE_SUFFIX}"
SELLER_ID="seller_${TEST_ID}_${CASE_SUFFIX}"
PRODUCT_ID="prod_${TEST_ID}_${CASE_SUFFIX}"
ORDER_ID="order_${TEST_ID}_${CASE_SUFFIX}"
ORDER_ITEM_ID="order_item_${TEST_ID}_${CASE_SUFFIX}"
...
psql "$DATABASE_URL" -c "INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'Tech Toys Store', 'Gadgets for all ages');" >/dev/null
psql "$DATABASE_URL" -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible) VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Robot Kit', 'Build your own robot', 'TOYS', 2499, 3, '[]'::jsonb, 'ACTIVE', true);" >/dev/null
...
psql "$DATABASE_URL" -c "INSERT INTO orders (id, buyer_id, status) VALUES ('${ORDER_ID}', '${BUYER_ID}', 'PAID');" >/dev/null
psql "$DATABASE_URL" -c "INSERT INTO order_items (id, order_id, product_id, qty, price_cents, seller_payout_cents) VALUES ('${ORDER_ITEM_ID}', '${ORDER_ID}', '${PRODUCT_ID}', 2, 2499, 1999);" >/dev/null
...
[ "$STATUS" = "200" ]
grep -F '"store_name":"Tech Toys Store"' "$RESPONSE_FILE" >/dev/null
grep -F '"bio":"Gadgets for all ages"' "$RESPONSE_FILE" >/dev/null
grep -F '"status":"ACTIVE"' "$RESPONSE_FILE" >/dev/null
grep -F '"title":"Robot Kit"' "$RESPONSE_FILE" >/dev/null
grep -F '"buyer_email":"' "$RESPONSE_FILE" >/dev/null
grep -F '"order_status":"PAID"' "$RESPONSE_FILE" >/dev/null
grep -F '"seller_payout_cents":1999' "$RESPONSE_FILE" >/dev/null
grep -F '"total_earnings_cents":1999' "$RESPONSE_FILE" >/dev/null

echo "CODEVALID_TEST_ASSERTION_OK:active_seller_dashboard_success"
