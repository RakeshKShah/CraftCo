...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="pending_seller_cannot_delete_products"
...
curl -sS -X POST "$BASE_URL/auth/register" \
  ...
  -d "{\"email\":\"${USER_EMAIL}\",\"password\":\"${USER_PASSWORD}\",\"role\":\"SELLER\",\"storeName\":\"Pending Store ${CASE_SUFFIX}\",\"bio\":\"pending seller test\"}" \
  > "$REGISTER_FILE"
TOKEN="$(jq -r '.token' "$REGISTER_FILE")"
SELLER_ID="$(jq -r '.user.sellerProfile.id' "$REGISTER_FILE")"
[ "$TOKEN" != "null" ]
psql "$DATABASE_URL" -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible) VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Pending Product ${CASE_SUFFIX}', 'should remain', 'tools', 2100, 3, '[]'::jsonb, 'ACTIVE', true);" >/dev/null
...
echo "CODEVALID_TEST_ASSERTION_OK:pending_seller_cannot_delete_products"
