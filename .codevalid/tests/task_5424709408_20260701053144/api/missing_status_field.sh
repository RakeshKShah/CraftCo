...
# Given — seed an admin and an existing seller for validation testing
...
VALUES ('${SELLER_ID}', '${SELLER_USER_ID}', 'Missing Status Store ${CASE_SUFFIX}', 'Missing field validation bio');
SQL
...
# When — omit the required status field from the update request body
...
# Then — API returns 400 and explains the missing required field
[ "$(cat "$STATUS_FILE")" = "400" ]
jq -e '.error | length > 0' "$UPDATE_JSON" >/dev/null
grep -E 'Required|expected|status' "$UPDATE_JSON" >/dev/null
[ "$(psql "$DATABASE_URL" -t -A -c "SELECT status FROM users WHERE id='${SELLER_USER_ID}';")" = "ACTIVE" ]

echo "CODEVALID_TEST_ASSERTION_OK:missing_status_field"
