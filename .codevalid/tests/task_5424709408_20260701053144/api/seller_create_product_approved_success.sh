...
CASE_SUFFIX="$(date +%s)-$$"
USER_ID="user-approved-${CASE_SUFFIX}"
SELLER_ID="seller-approved-${CASE_SUFFIX}"
EMAIL="approved-${CASE_SUFFIX}@example.com"
TITLE="Handmade Mug ${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/seller_create_product_approved_success_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/seller_create_product_approved_success_${CASE_SUFFIX}.status"
TOKEN_FILE="/tmp/seller_create_product_approved_success_${CASE_SUFFIX}.token"
PRODUCT_ID=""
cleanup() {
...
psql "$DATABASE_URL" -c "INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'Approved Store ${CASE_SUFFIX}', 'Ceramic seller ${CASE_SUFFIX}');"
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  ...
  --data "{\"title\":\"${TITLE}\",\"description\":\"Ceramic mug with handle\",\"category\":\"HOME_GOODS\",\"price_cents\":3500,\"stock_qty\":10,\"photos\":[\"https://example.com/mug.jpg\"]}" > "$STATUS_FILE"
...
grep -F '"description":"Ceramic mug with handle"' "$RESPONSE_FILE" >/dev/null
grep -F '"category":"HOME_GOODS"' "$RESPONSE_FILE" >/dev/null
grep -F '"priceCents":3500' "$RESPONSE_FILE" >/dev/null
...
[ -n "$PRODUCT_ID" ]
DB_COUNT="$(psql "$DATABASE_URL" -t -A -c "SELECT COUNT(*) FROM products WHERE id='${PRODUCT_ID}' AND seller_id='${SELLER_ID}' AND title='${TITLE}' AND price_cents=3500 AND stock_qty=10 AND status='ACTIVE' AND visible=true;")"
[ "$DB_COUNT" = "1" ]
echo "CODEVALID_TEST_ASSERTION_OK:seller_create_product_approved_success"
