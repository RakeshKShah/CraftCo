...
SELLER_ID="seller-pending-${CASE_SUFFIX}"
EMAIL="pending-${CASE_SUFFIX}@example.com"
TITLE="Art Print ${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/pending_seller_cannot_create_product_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/pending_seller_cannot_create_product_${CASE_SUFFIX}.status"
TOKEN_FILE="/tmp/pending_seller_cannot_create_product_${CASE_SUFFIX}.token"
cleanup() {
...
psql "$DATABASE_URL" -c "INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'Pending Store ${CASE_SUFFIX}', 'Awaiting approval');"
node -e 'const jwt=require("jsonwebtoken"); process.stdout.write(jwt.sign({id:process.argv[1],email:process.argv[2],role:"SELLER",status:"PENDING",sellerProfileId:process.argv[3]}, process.env.JWT_SECRET));' "$USER_ID" "$EMAIL" "$SELLER_ID" > "$TOKEN_FILE"
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  ...
  --data "{\"title\":\"${TITLE}\",\"description\":\"Pending item\",\"category\":\"ART\",\"price_cents\":5000,\"stock_qty\":3,\"photos\":[\"https://example.com/art.jpg\"]}" > "$STATUS_FILE"
