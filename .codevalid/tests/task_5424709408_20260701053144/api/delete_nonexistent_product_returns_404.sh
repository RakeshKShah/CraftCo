...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="delete_nonexistent_product_returns_404"
...
curl -sS -X POST "$BASE_URL/auth/register" \
  ...
  -d "{\"email\":\"${USER_EMAIL}\",\"password\":\"${USER_PASSWORD}\",\"role\":\"SELLER\",\"storeName\":\"404 Store ${CASE_SUFFIX}\",\"bio\":\"missing product test\"}" > "$REGISTER_FILE"
...
COUNT_BEFORE="$(psql "$DATABASE_URL" -t -A -c "SELECT COUNT(*) FROM products WHERE id = '${MISSING_PRODUCT_ID}';")"
[ "$COUNT_BEFORE" = "0" ]

# When
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X DELETE \
  "$BASE_URL/products/${MISSING_PRODUCT_ID}" \
...
COUNT_AFTER="$(psql "$DATABASE_URL" -t -A -c "SELECT COUNT(*) FROM products WHERE id = '${MISSING_PRODUCT_ID}';")"
[ "$COUNT_AFTER" = "0" ]

echo "CODEVALID_TEST_ASSERTION_OK:delete_nonexistent_product_returns_404"
