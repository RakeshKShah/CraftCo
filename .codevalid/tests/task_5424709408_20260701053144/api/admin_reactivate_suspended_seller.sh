...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="admin_reactivate_suspended_seller"
...
SELLER_ID="seller-suspended456-${CASE_SUFFIX}"
PRODUCT_ID_1="reactivate-prod-1-${CASE_SUFFIX}"
PRODUCT_ID_2="reactivate-prod-2-${CASE_SUFFIX}"
...
# Given — seed an admin and a suspended seller with hidden products
...
INSERT INTO users (id, email, password_hash, role, status, created_at)
VALUES ('${SELLER_USER_ID}', '${SELLER_EMAIL}', '${HASH}', 'SELLER', 'SUSPENDED', NOW());
INSERT INTO seller_profiles (id, user_id, store_name, bio)
VALUES ('${SELLER_ID}', '${SELLER_USER_ID}', 'Reactivated Store ${CASE_SUFFIX}', 'Suspended bio ${CASE_SUFFIX}');
INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible, created_at)
VALUES
  ('${PRODUCT_ID_1}', '${SELLER_ID}', 'Hidden Product 1 ${CASE_SUFFIX}', 'Hidden while suspended', 'tools', 1800, 2, '[]', 'ACTIVE', false, NOW()),
  ('${PRODUCT_ID_2}', '${SELLER_ID}', 'Hidden Product 2 ${CASE_SUFFIX}', 'Hidden while suspended', 'tools', 2800, 4, '[]', 'ACTIVE', false, NOW());
SQL
...
# Then — response is 200, user is ACTIVE, and hidden products are restored to marketplace visibility
[ "$(cat "$STATUS_FILE")" = "200" ]
...
jq -e --arg p "$PRODUCT_ID_1" '.[] | select(.id == $p)' "$MARKET_JSON" >/dev/null
jq -e --arg p "$PRODUCT_ID_2" '.[] | select(.id == $p)' "$MARKET_JSON" >/dev/null

echo "CODEVALID_TEST_ASSERTION_OK:admin_reactivate_suspended_seller"

# Cleanup — remove seeded seller, products, and admin
