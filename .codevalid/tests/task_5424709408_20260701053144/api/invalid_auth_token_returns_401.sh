...
RESPONSE_FILE="/tmp/invalid_auth_token_returns_401_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/invalid_auth_token_returns_401_${CASE_SUFFIX}.status"
cleanup() {
...
psql "$DATABASE_URL" -c "INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'Invalid Token Store ${CASE_SUFFIX}', 'Invalid token bio ${CASE_SUFFIX}');"
psql "$DATABASE_URL" -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible) VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Invalid Token Original', 'Invalid token description', 'SPORTS', 2100, 4, '[]'::jsonb, 'ACTIVE', true);"
...
[ "$DB_TITLE" = 'Invalid Token Original' ]
echo "CODEVALID_TEST_ASSERTION_OK:invalid_auth_token_returns_401"
