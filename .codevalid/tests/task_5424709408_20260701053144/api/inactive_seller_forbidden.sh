...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="inactive_seller_forbidden"
...
curl -sS -X POST "$BASE_URL/auth/register" \
  ...
  -d "{\"email\":\"${USER_EMAIL}\",\"password\":\"${USER_PASSWORD}\",\"role\":\"SELLER\",\"storeName\":\"Pending Store ${CASE_SUFFIX}\",\"bio\":\"pending seller\"}" \
  > "$REGISTER_FILE"
TOKEN="$(jq -r '.token' "$REGISTER_FILE")"
USER_ID="$(jq -r '.user.id' "$REGISTER_FILE")"
SELLER_ID="$(jq -r '.user.sellerProfile.id' "$REGISTER_FILE")"
[ "$TOKEN" != "null" ]
psql "$DATABASE_URL" -c "UPDATE users SET status = 'PENDING' WHERE id = '${USER_ID}';" >/dev/null
psql "$DATABASE_URL" -c "INSERT INTO products (id, seller_id, title, description, category, price_cents, stock_qty, photos, status, visible) VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Pending Delete ${CASE_SUFFIX}', 'pending seller product', 'tools', 1750, 2, '[]'::jsonb, 'ACTIVE', true);" >/dev/null
