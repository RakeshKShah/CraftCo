...
SELLER_ID="seller-profile-suspended-${CASE_SUFFIX}"
EMAIL="suspended-${CASE_SUFFIX}@example.com"
TOKEN_FILE="/tmp/suspended_seller_cannot_create_products_${CASE_SUFFIX}.token"
...
cleanup() {
  psql "$DATABASE_URL" -c "DELETE FROM products WHERE title='Suspended Product ${CASE_SUFFIX}';" >/dev/null 2>&1 || true
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  ...
  --data "{\"title\":\"Suspended Product ${CASE_SUFFIX}\",\"description\":\"Minimal\",\"category\":\"MISC\",\"price_cents\":100,\"stock_qty\":1,\"photos\":[]}" > "$STATUS_FILE"

# Then
STATUS="$(cat "$STATUS_FILE")"
[ "$STATUS" = "403" ]
grep -F '"error":"Suspended sellers cannot create products"' "$RESPONSE_FILE" >/dev/null
DB_COUNT="$(psql "$DATABASE_URL" -t -A -c "SELECT COUNT(*) FROM products WHERE title='Suspended Product ${CASE_SUFFIX}';")"
[ "$DB_COUNT" = "0" ]
echo "CODEVALID_TEST_ASSERTION_OK:suspended_seller_cannot_create_products"
