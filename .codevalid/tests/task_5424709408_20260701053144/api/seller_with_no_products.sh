...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="seller_with_no_products"
...
psql "$DATABASE_URL" -c "INSERT INTO seller_profiles (id, user_id, store_name, bio) VALUES ('${SELLER_ID}', '${USER_ID}', 'Future Store', 'Coming soon');" >/dev/null
...
[ "$STATUS" = "200" ]
grep -F '"store_name":"Future Store"' "$RESPONSE_FILE" >/dev/null
grep -F '"bio":"Coming soon"' "$RESPONSE_FILE" >/dev/null
grep -F '"status":"ACTIVE"' "$RESPONSE_FILE" >/dev/null
grep -F '"products":[]' "$RESPONSE_FILE" >/dev/null
grep -F '"orders":[]' "$RESPONSE_FILE" >/dev/null
grep -F '"total_earnings_cents":0' "$RESPONSE_FILE" >/dev/null

echo "CODEVALID_TEST_ASSERTION_OK:seller_with_no_products"
