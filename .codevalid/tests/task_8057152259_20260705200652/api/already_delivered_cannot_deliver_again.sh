...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="already_delivered_cannot_deliver_again"
BUYER_EMAIL="buyer-already-delivered-${CASE_SUFFIX}@example.com"
BUYER_PASSWORD="Password123!"
BUYER_ID="buyer_already_delivered_${CASE_SUFFIX}"
ORDER_ID="order_already_delivered_${CASE_SUFFIX}"
...
psql "$DATABASE_URL" -c "INSERT INTO orders (id, buyer_id, status, total_cents, platform_fee_cents) VALUES ('${ORDER_ID}', '${BUYER_ID}', 'DELIVERED', 2600, 260);" >/dev/null
...
[ "$DB_STATUS" = "DELIVERED" ]
echo "CODEVALID_TEST_ASSERTION_OK:already_delivered_cannot_deliver_again"
