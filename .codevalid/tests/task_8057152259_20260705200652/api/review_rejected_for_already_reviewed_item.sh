...
ORDER_ITEM_ID="order-item-review-duplicate-${CASE_SUFFIX}"
EXISTING_REVIEW_ID="review-existing-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/review_rejected_for_already_reviewed_item_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/review_rejected_for_already_reviewed_item_${CASE_SUFFIX}.status"
...
VALUES ('${PRODUCT_ID}', '${SELLER_ID}', 'Reviewed Product ${CASE_SUFFIX}', 'Fixture for duplicate review rejection', 2799, 7, 'ACTIVE', true, NOW());

INSERT INTO orders (id, buyer_id, status, total_cents, created_at)
VALUES ('${ORDER_ID}', '${BUYER_ID}', 'DELIVERED', 2799, NOW());

INSERT INTO order_items (id, order_id, product_id, quantity, unit_price_cents)
VALUES ('${ORDER_ITEM_ID}', '${ORDER_ID}', '${PRODUCT_ID}', 1, 2799);

INSERT INTO reviews (id, order_item_id, product_id, buyer_id, rating, body, created_at)
VALUES ('${EXISTING_REVIEW_ID}', '${ORDER_ITEM_ID}', '${PRODUCT_ID}', '${BUYER_ID}', 5, 'First review already stored', NOW());
SQL
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST "$BASE_URL/api/reviews" \
  ...
  --data "{\"order_item_id\":\"${ORDER_ITEM_ID}\",\"rating\":2,\"body\":\"Not satisfied\"}" \
  > "$STATUS_FILE"
...
echo "CODEVALID_TEST_ASSERTION_OK:review_rejected_for_already_reviewed_item"
