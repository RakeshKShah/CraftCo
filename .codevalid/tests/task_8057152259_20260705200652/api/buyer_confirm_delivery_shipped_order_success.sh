...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="buyer_confirm_delivery_shipped_order_success"
BUYER_EMAIL="buyer-deliver-${CASE_SUFFIX}@example.com"
BUYER_PASSWORD="Password123!"
BUYER_ID="buyer_deliver_${CASE_SUFFIX}"
ORDER_ID="order_deliver_${CASE_SUFFIX}"
...
psql "$DATABASE_URL" -c "INSERT INTO orders (id, buyer_id, status, total_cents, platform_fee_cents) VALUES ('${ORDER_ID}', '${BUYER_ID}', 'SHIPPED', 1500, 150);" >/dev/null
...
[ "$DB_STATUS" = "DELIVERED" ]
echo "CODEVALID_TEST_ASSERTION_OK:buyer_confirm_delivery_shipped_order_success"
