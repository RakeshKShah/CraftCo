...
CASE_SUFFIX="$(date +%s)-$$"
BUYER_ID="buyer-lowstock-${CASE_SUFFIX}"
SELLER_USER_ID="seller-user-lowstock-${CASE_SUFFIX}"
SELLER_PROFILE_ID="seller-profile-lowstock-${CASE_SUFFIX}"
PRODUCT_ID="prod-lowstock-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/checkout_fails_insufficient_stock_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/checkout_fails_insufficient_stock_${CASE_SUFFIX}.status"
...
INSERT INTO "SellerProfile" (id, "userId", "storeName", bio, status, "createdAt") VALUES
  ('${SELLER_PROFILE_ID}', '${SELLER_USER_ID}', 'Low Stock Store ${CASE_SUFFIX}', 'bio', 'ACTIVE', NOW());
INSERT INTO "Product" (id, title, description, price_cents, stock_qty, visible, status, "sellerId", category, "createdAt") VALUES
  ('${PRODUCT_ID}', 'Low Stock Product ${CASE_SUFFIX}', 'desc', 1999, 3, true, 'ACTIVE', '${SELLER_PROFILE_ID}', 'general', NOW());
SQL
...
[ "$STOCK_NOW" = '3' ]
echo 'CODEVALID_TEST_ASSERTION_OK:checkout_fails_insufficient_stock'
