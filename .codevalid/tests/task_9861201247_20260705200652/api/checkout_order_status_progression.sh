...
CASE_SUFFIX="$(date +%s)-$$"
BUYER_ID="buyer-status-${CASE_SUFFIX}"
SELLER_USER_ID="seller-user-status-${CASE_SUFFIX}"
SELLER_PROFILE_ID="seller-profile-status-${CASE_SUFFIX}"
PRODUCT_ID="prod-status-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/checkout_order_status_progression_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/checkout_order_status_progression_${CASE_SUFFIX}.status"
ORDER_ID=""
...
INSERT INTO "SellerProfile" (id, "userId", "storeName", bio, status, "createdAt") VALUES
  ('${SELLER_PROFILE_ID}', '${SELLER_USER_ID}', 'Status Store ${CASE_SUFFIX}', 'bio', 'ACTIVE', NOW());
INSERT INTO "Product" (id, title, description, price_cents, stock_qty, visible, status, "sellerId", category, "createdAt") VALUES
  ('${PRODUCT_ID}', 'Status Product ${CASE_SUFFIX}', 'desc', 1999, 10, true, 'ACTIVE', '${SELLER_PROFILE_ID}', 'general', NOW());
SQL
...
[ -n "$ORDER_ID" ]
FINAL_STATUS="$(psql "$DATABASE_URL" -At -c "SELECT status FROM \"Order\" WHERE id = '${ORDER_ID}';")"
[ "$FINAL_STATUS" = 'PAID' ]
PENDING_COUNT="$(psql "$DATABASE_URL" -At -c "SELECT COUNT(*) FROM \"Order\" WHERE id = '${ORDER_ID}' AND status = 'PENDING';")"
[ "$PENDING_COUNT" = '0' ]
echo 'CODEVALID_TEST_ASSERTION_OK:checkout_order_status_progression'
