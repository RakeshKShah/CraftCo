...
CASE_SUFFIX="$(date +%s)-$$"
BUYER_ID="buyer-ship-${CASE_SUFFIX}"
SELLER_USER_ID="seller-user-ship-${CASE_SUFFIX}"
SELLER_PROFILE_ID="seller-profile-ship-${CASE_SUFFIX}"
PRODUCT_ID="prod-ship-${CASE_SUFFIX}"
ORDER_ID="order-ship-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/seller_marks_paid_order_shipped_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/seller_marks_paid_order_shipped_${CASE_SUFFIX}.status"
...
INSERT INTO "SellerProfile" (id, "userId", "storeName", bio, status, "createdAt") VALUES
  ('${SELLER_PROFILE_ID}', '${SELLER_USER_ID}', 'Shipping Store ${CASE_SUFFIX}', 'bio', 'ACTIVE', NOW());
INSERT INTO "Product" (id, title, description, price_cents, stock_qty, visible, status, "sellerId", category, "createdAt") VALUES
  ('${PRODUCT_ID}', 'Shipping Product ${CASE_SUFFIX}', 'desc', 1999, 8, true, 'ACTIVE', '${SELLER_PROFILE_ID}', 'general', NOW());
...
INSERT INTO "OrderItem" (id, "orderId", "productId", "sellerId", qty, price_at_purchase, seller_payout_cents) VALUES
  ('oi-ship-${CASE_SUFFIX}', '${ORDER_ID}', '${PRODUCT_ID}', '${SELLER_PROFILE_ID}', 1, 1999, 1799);
SQL
...
[ "$DB_STATUS" = 'SHIPPED' ]
echo 'CODEVALID_TEST_ASSERTION_OK:seller_marks_paid_order_shipped'
