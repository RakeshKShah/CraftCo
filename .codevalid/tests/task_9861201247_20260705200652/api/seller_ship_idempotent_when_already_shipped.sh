...
CASE_SUFFIX="$(date +%s)-$$"
BUYER_ID="buyer-alreadyshipped-${CASE_SUFFIX}"
SELLER_USER_ID="seller-user-alreadyshipped-${CASE_SUFFIX}"
SELLER_PROFILE_ID="seller-profile-alreadyshipped-${CASE_SUFFIX}"
PRODUCT_ID="prod-alreadyshipped-${CASE_SUFFIX}"
ORDER_ID="order-alreadyshipped-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/seller_ship_idempotent_when_already_shipped_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/seller_ship_idempotent_when_already_shipped_${CASE_SUFFIX}.status"
...
INSERT INTO "SellerProfile" (id, "userId", "storeName", bio, status, "createdAt") VALUES
  ('${SELLER_PROFILE_ID}', '${SELLER_USER_ID}', 'Already Shipped Store ${CASE_SUFFIX}', 'bio', 'ACTIVE', NOW());
INSERT INTO "Product" (id, title, description, price_cents, stock_qty, visible, status, "sellerId", category, "createdAt") VALUES
  ('${PRODUCT_ID}', 'Already Shipped Product ${CASE_SUFFIX}', 'desc', 1999, 8, true, 'ACTIVE', '${SELLER_PROFILE_ID}', 'general', NOW());
...
INSERT INTO "OrderItem" (id, "orderId", "productId", "sellerId", qty, price_at_purchase, seller_payout_cents) VALUES
  ('oi-alreadyshipped-${CASE_SUFFIX}', '${ORDER_ID}', '${PRODUCT_ID}', '${SELLER_PROFILE_ID}', 1, 1999, 1799);
SQL
...
[ "$DB_STATUS" = 'SHIPPED' ]
echo 'CODEVALID_TEST_ASSERTION_OK:seller_ship_idempotent_when_already_shipped'
