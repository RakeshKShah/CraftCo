...
SELLER_PROFILE_ID="seller-profile-notify-${CASE_SUFFIX}"
PRODUCT_ID="prod-notify-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/successful_checkout_notifies_seller_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/successful_checkout_notifies_seller_${CASE_SUFFIX}.status"
ORDER_ID=""
...
INSERT INTO "SellerProfile" (id, "userId", "storeName", bio, status, "createdAt") VALUES
  ('${SELLER_PROFILE_ID}', '${SELLER_USER_ID}', 'Notify Store ${CASE_SUFFIX}', 'bio', 'ACTIVE', NOW());
INSERT INTO "Product" (id, title, description, price_cents, stock_qty, visible, status, "sellerId", category, "createdAt") VALUES
  ('${PRODUCT_ID}', 'Notify Product ${CASE_SUFFIX}', 'desc', 1999, 10, true, 'ACTIVE', '${SELLER_PROFILE_ID}', 'general', NOW());
SQL
...
[ -n "$ORDER_ID" ]
ORDER_ROW="$(psql "$DATABASE_URL" -At -F '|' -c "SELECT total_cents, status, buyer_id FROM \"Order\" WHERE id = '${ORDER_ID}';")"
[ "$ORDER_ROW" = "3998|PAID|${BUYER_ID}" ]
ORDER_ITEM_ROW="$(psql "$DATABASE_URL" -At -F '|' -c "SELECT \"sellerId\", qty, price_at_purchase FROM \"OrderItem\" WHERE \"orderId\" = '${ORDER_ID}' AND \"productId\" = '${PRODUCT_ID}';")"
[ "$ORDER_ITEM_ROW" = "${SELLER_PROFILE_ID}|2|1999" ]
UPDATED_STOCK="$(psql "$DATABASE_URL" -At -c "SELECT stock_qty FROM \"Product\" WHERE id = '${PRODUCT_ID}';")"
[ "$UPDATED_STOCK" = '8' ]
# Resend delivery is a documented no-op in this environment; assert the seller-linked paid order item exists.
echo 'CODEVALID_TEST_ASSERTION_OK:successful_checkout_notifies_seller'
