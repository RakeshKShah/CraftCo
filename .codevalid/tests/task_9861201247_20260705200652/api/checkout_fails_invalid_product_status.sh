...
CASE_SUFFIX="$(date +%s)-$$"
BUYER_ID="buyer-draft-${CASE_SUFFIX}"
SELLER_USER_ID="seller-user-draft-${CASE_SUFFIX}"
SELLER_PROFILE_ID="seller-profile-draft-${CASE_SUFFIX}"
PRODUCT_ID="prod-draft-${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/checkout_fails_invalid_product_status_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/checkout_fails_invalid_product_status_${CASE_SUFFIX}.status"
...
INSERT INTO "SellerProfile" (id, "userId", "storeName", bio, status, "createdAt") VALUES
  ('${SELLER_PROFILE_ID}', '${SELLER_USER_ID}', 'Draft Store ${CASE_SUFFIX}', 'bio', 'ACTIVE', NOW());
INSERT INTO "Product" (id, title, description, price_cents, stock_qty, visible, status, "sellerId", category, "createdAt") VALUES
  ('${PRODUCT_ID}', 'Draft Product ${CASE_SUFFIX}', 'desc', 1999, 10, true, 'DRAFT', '${SELLER_PROFILE_ID}', 'general', NOW());
SQL
...
[ "$STOCK_NOW" = '10' ]
echo 'CODEVALID_TEST_ASSERTION_OK:checkout_fails_invalid_product_status'
