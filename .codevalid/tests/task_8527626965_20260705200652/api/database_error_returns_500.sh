...
CASE_SUFFIX="$(date +%s)-$$"
ADMIN_EMAIL="db-error-admin-${CASE_SUFFIX}@example.com"
ADMIN_PASSWORD='AdminPass123!'
SELLER_EMAIL="db-error-seller-${CASE_SUFFIX}@example.com"
SELLER_PASSWORD='SellerPass123!'
TMP_DIR="$(mktemp -d)"
ADMIN_RESP="$TMP_DIR/admin.json"
SELLER_RESP="$TMP_DIR/seller.json"
RESP_FILE="$TMP_DIR/db-error.json"
STATUS_FILE="$TMP_DIR/db-error.status"
TOXIC_NAME="seller-update-timeout-${CASE_SUFFIX}"
cleanup() {
  curl -sS -X DELETE "http://toxiproxy:8474/proxies/postgres/toxics/${TOXIC_NAME}" >/dev/null 2>&1 || true
...
curl -sS -o "$SELLER_RESP" -w '%{http_code}' -X POST "$BASE_URL/auth/register" \
  -H 'Content-Type: application/json' \
  --data "{\"email\":\"${SELLER_EMAIL}\",\"password\":\"${SELLER_PASSWORD}\",\"role\":\"SELLER\",\"storeName\":\"DB Error Store ${CASE_SUFFIX}\",\"bio\":\"bio\"}" > "$TMP_DIR/seller.status"
...
curl -sS -X POST "http://toxiproxy:8474/proxies/postgres/toxics" \
  -H 'Content-Type: application/json' \
  --data "{\"name\":\"${TOXIC_NAME}\",\"type\":\"timeout\",\"stream\":\"downstream\",\"attributes\":{\"timeout\":5000}}" > /dev/null

# When
curl -sS -o "$RESP_FILE" -w '%{http_code}' -X PUT "$BASE_URL/admin/sellers/${SELLER_ID}" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer ${ADMIN_TOKEN}" \
  --data '{"status":"SUSPENDED"}' > "$STATUS_FILE" || true
curl -sS -X DELETE "http://toxiproxy:8474/proxies/postgres/toxics/${TOXIC_NAME}" >/dev/null 2>&1 || true

# Then
[ "$(cat "$STATUS_FILE")" = "500" ]
grep -F '"error":"Update failed"' "$RESP_FILE" >/dev/null
if grep -E 'stack|Error:' "$RESP_FILE" >/dev/null 2>&1; then
  exit 1
fi
