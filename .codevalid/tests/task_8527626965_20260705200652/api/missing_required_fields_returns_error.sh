...
REGISTER_RESPONSE="/tmp/missing_required_fields_returns_error_register_${CASE_SUFFIX}.json"
RESPONSE_FILE="/tmp/missing_required_fields_returns_error_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/missing_required_fields_returns_error_${CASE_SUFFIX}.status"
cleanup_files() {
...
curl -sS -X POST "$BASE_URL/register" \
  ...
  -d "{\"email\":\"${EMAIL}\",\"password\":\"Password123!\",\"role\":\"SELLER\",\"storeName\":\"Missing Fields Shop ${CASE_SUFFIX}\",\"bio\":\"Validation test\"}" \
  > "$REGISTER_RESPONSE"
...
echo "CODEVALID_TEST_ASSERTION_OK:missing_required_fields_returns_error"
