...
CASE_SUFFIX="$(date +%s)-$$"
ADMIN_EMAIL="missing-status-admin-${CASE_SUFFIX}@example.com"
ADMIN_PASSWORD='AdminPass123!'
SELLER_EMAIL="missing-status-seller-${CASE_SUFFIX}@example.com"
SELLER_PASSWORD='SellerPass123!'
TMP_DIR="$(mktemp -d)"
ADMIN_RESP="$TMP_DIR/admin.json"
SELLER_RESP="$TMP_DIR/seller.json"
RESP_FILE="$TMP_DIR/missing-status.json"
STATUS_FILE="$TMP_DIR/missing-status.status"
cleanup() {
...
curl -sS -o "$SELLER_RESP" -w '%{http_code}' -X POST "$BASE_URL/auth/register" \
  -H 'Content-Type: application/json' \
  --data "{\"email\":\"${SELLER_EMAIL}\",\"password\":\"${SELLER_PASSWORD}\",\"role\":\"SELLER\",\"storeName\":\"Missing Status Store ${CASE_SUFFIX}\",\"bio\":\"bio\"}" > "$TMP_DIR/seller.status"
...
[ "$DB_STATUS" = "PENDING" ]
echo "CODEVALID_TEST_ASSERTION_OK:missing_status_returns_400"
