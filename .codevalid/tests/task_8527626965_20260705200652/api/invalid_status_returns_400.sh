...
CASE_SUFFIX="$(date +%s)-$$"
ADMIN_EMAIL="invalid-status-admin-${CASE_SUFFIX}@example.com"
ADMIN_PASSWORD='AdminPass123!'
SELLER_EMAIL="invalid-status-seller-${CASE_SUFFIX}@example.com"
SELLER_PASSWORD='SellerPass123!'
TMP_DIR="$(mktemp -d)"
ADMIN_RESP="$TMP_DIR/admin.json"
SELLER_RESP="$TMP_DIR/seller.json"
RESP_FILE="$TMP_DIR/invalid-status.json"
STATUS_FILE="$TMP_DIR/invalid-status.status"
cleanup() {
...
curl -sS -o "$SELLER_RESP" -w '%{http_code}' -X POST "$BASE_URL/auth/register" \
  -H 'Content-Type: application/json' \
  --data "{\"email\":\"${SELLER_EMAIL}\",\"password\":\"${SELLER_PASSWORD}\",\"role\":\"SELLER\",\"storeName\":\"Invalid Status Store ${CASE_SUFFIX}\",\"bio\":\"bio\"}" > "$TMP_DIR/seller.status"
[ "$(cat "$TMP_DIR/seller.status")" = "201" ]
SELLER_ID="$(jq -r '.user.sellerProfile.id' "$SELLER_RESP")"
USER_ID="$(jq -r '.user.id' "$SELLER_RESP")"
ORIGINAL_STATUS="$(psql "$DATABASE_URL" -At -c "SELECT status FROM \"User\" WHERE id='${USER_ID}';")"
...
curl -sS -o "$RESP_FILE" -w '%{http_code}' -X PUT "$BASE_URL/admin/sellers/${SELLER_ID}" \
  ...
  --data '{"status":"BANNED"}' > "$STATUS_FILE"

# Then
[ "$(cat "$STATUS_FILE")" = "400" ]
grep -F 'error' "$RESP_FILE" >/dev/null
UPDATED_STATUS="$(psql "$DATABASE_URL" -At -c "SELECT status FROM \"User\" WHERE id='${USER_ID}';")"
[ "$UPDATED_STATUS" = "$ORIGINAL_STATUS" ]
echo "CODEVALID_TEST_ASSERTION_OK:invalid_status_returns_400"
